...
if [[ "$CURRENT_CRED" != "$NEW_CRED" ]]; then
	...
	wget --user=$USER --password=$PASS https://cdn.mailcleaner.net/downloads/bayes/bayes_toks.md5 -P /tmp/ >/dev/null 2>&1
	...
	if [[ "$CURRENT_MD5" != "$NEW_MD5" ]]; then
		wget --user=$USER --password=$PASS https://cdn.mailcleaner.net/downloads/bayes/bayes_toks -P /tmp/ >/dev/null 2>&1
	...
	wget --user=$USER --password=$PASS https://cdn.mailcleaner.net/downloads/bayes/wordlist.db.md5 -P /tmp/ >/dev/null 2>&1
	...
	# If MD5 changed then the associated bayesians have to be updated
	if [[ "$CURRENT_MD5" != "$NEW_MD5" ]]; then
		wget --user=$USER --password=$PASS https://cdn.mailcleaner.net/downloads/bayes/wordlist.db -P /tmp/ >/dev/null 2>&1
