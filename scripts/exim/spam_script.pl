...
# if we want to test the address before putting a spam in quarantine
my $test_address_mode = 0;
...
sub put_in_quarantine
{
	...
	my $in_master = 1;
	if ( defined $config_dbh ) {
		my $master_sth = $config_dbh->prepare("SELECT hostname, port, password FROM master"
        ) or return 0;
