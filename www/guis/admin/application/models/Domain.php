...
class Default_Model_Domain
{
    ...
    public function getSummaryFrequency()
    {
        if ($this->getPref('daily_summary')) {
            return $this->_summaryFrequencies['daily'];
        }
        if ($this->getPref('weekly_summary')) {
            return $this->_summaryFrequencies['weekly'];
        }
        if ($this->getPref('monthly_summary')) {
            return $this->_summaryFrequencies['monthly'];
        }
    ...
    public function setSummaryFrequency($frequency)
    {
        ...
        if ($frequency == 2 || $frequency == 'weekly') {
            $this->setPref('weekly_summary', '1');
        }
        if ($frequency == 3 || $frequency == 'monthly') {
    ...
    public function testUserAuth($username, $password)
    {
        ...
        $_POST['username'] = iconv_mime_decode($username, ICONV_MIME_DECODE_CONTINUE_ON_ERROR, 'UTF-8');
        $_POST['password'] = iconv_mime_decode($password, ICONV_MIME_DECODE_CONTINUE_ON_ERROR, 'UTF-8');
