...
if (preg_match('/^\@([a-zA-Z0-9\_\-\.]+)$/', $address, $matches) && in_array($matches[1], $sysconf_->getFilteredDomains()) && $admin_->canManageDomain($matches[1])) {
	$mode = 'domain';
}
if (preg_match('/^([a-zA-Z0-9\_\-\.]+)\@([a-zA-Z0-9\_\-\.]+)$/', $address, $matches)) {
	$mode = "address";
}
if ($mode == "badmode") {
